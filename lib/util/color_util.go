...
func makeRainbowTable() {
	...
	rainbowTable = make([]Color32, 100+1)
...
// GetRainbow param: 0.0〜1.0
func GetRainbow(param float64) Color32 {
	if rainbowTable == nil {
		makeRainbowTable()
	}
	index := int(param * 100)
	// if index >= len(rainbowTable) {
	// 	log.Error("Invalid Index:", index)
	// }
	return rainbowTable[index]
}
...
func (c *HSV) RGB() *RGB {
	...
	ir, ig, ib := uint8(r), uint8(g), uint8(b)
	rgb := &RGB{R:ir, G:ig, B:ib, rgb:ToUint32(ir, ig, ib)}
	return rgb
